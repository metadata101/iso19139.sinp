  ...
  <xsl:template name="get-iso19139.sinp-extents-as-json">[
    <xsl:for-each select="//gmd:geographicElement/gmd:EX_GeographicBoundingBox">
      ...
      <xsl:if test="number(gmd:westBoundLongitude/gco:Decimal)
        ...
        [
        <xsl:value-of
                select="format-number(gmd:westBoundLongitude/gco:Decimal, $format)"/>,
        <xsl:value-of
                select="format-number(gmd:southBoundLatitude/gco:Decimal, $format)"/>,
        <xsl:value-of
                select="format-number(gmd:eastBoundLongitude/gco:Decimal, $format)"/>,
        <xsl:value-of
                select="format-number(gmd:northBoundLatitude/gco:Decimal, $format)"/>
        ]
